...
def build_json_ld(article: dict, canonical_url: str, thumb_url: str, date: str, platform: str = "github") -> str:
    ...
    if products:
        graph_nodes.append({
            ...
            "itemListElement": [
                {"@type": "ListItem", "position": i, "item": {"@type": "Product", "name": p.get("name", ""), "description": p.get("description", "")}}
...
def _fetch_thumbnail_bg(scene: str, size: tuple, seed: int):
    ...
    try:
        ...
        print(f"  → [썸네일 아트워크] 콜라주 배경 생성 실패, 그라데이션만 사용: {e}")
        return None
...
def enhance_tables(html_body: str, accent: str) -> str:
    ...
    def wrap_table(match):
        ...
        return (
            f'<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;margin:1.2em 0 0.4em;border-radius:8px;border:1px solid #eee;">{styled_table}</div>'
            f'<div style="text-align:right;margin:0 0 1.2em;"><button type="button" onclick="document.getElementById(\'{uid}\').style.display=\'flex\';" style="border:none;background:none;color:{accent};font-size:0.85em;font-weight:700;cursor:pointer;padding:4px 2px;">🔍 표 크게 보기</button></div>'
            f'<div id="{uid}" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.82);z-index:1000;align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this){{this.style.display=\'none\';}}">'
            f'<div style="background:#fff;border-radius:12px;padding:18px;max-width:95vw;max-height:88vh;overflow:auto;">'
            f'<button type="button" onclick="document.getElementById(\'{uid}\').style.display=\'none\';" style="display:block;margin:0 0 10px auto;width:32px;height:32px;border-radius:50%;border:none;background:#f0f0f0;font-size:1em;cursor:pointer;">✕</button>'
            f'{modal_table}</div></div>'
        )
...
def save_post(article: dict):
    ...
    html = POST_TEMPLATE.format(
        title=title, meta_description=meta_description, date=today, html_body=article["html_body"],
        thumb_filename=thumb_filename, canonical_url=post_url, thumb_url=thumb_url, json_ld=json_ld,
        ga_snippet=_ga_snippet(), adsense_snippet=_adsense_snippet(), font=theme["font"],
        font_family=_font_family_name(theme["font"]), accent=theme["accent"], badge=theme["badge"],
        related_html=related_html, post_nav=post_nav, decor_html=decor_html, bottom_ad=_manual_ad_unit(),
...
def update_index(new_post: dict) -> list:
    ...
    with open(os.path.join(DOCS_DIR, "index.html"), "w", encoding="utf-8") as f:
        ...
        f.write(INDEX_TEMPLATE.format(
            ...
            adsense_snippet=_adsense_snippet(), fonts_url=_google_fonts_url(), hero_html=hero_html,
            mid_html=mid_html, bottom_html=bottom_html, blog_json_ld=build_blog_index_json_ld(posts),
            category_pills=category_pills, search_console_meta=_search_console_meta(),
            footer_html=build_footer_html(), translate_widget=_translate_widget(),
        ))
    return posts
...
def build_footer_html() -> str:
    return (
        ...
        '<a href="about.html">블로그 소개</a>·<a href="privacy.html">개인정보처리방침</a>·<a href="contact.html">문의하기</a>'
...
def generate_static_pages() -> None:
    ...
    pages = {
        "about.html": (
            ...
            "<p>이 블로그는 다양한 주제의 정보를 정리해서 소개하며, 콘텐츠 제작 과정 일부에 AI 도구를 활용하고 있습니다. 게시된 정보는 참고용이며, 중요한 결정을 내리실 때는 반드시 공식 출처를 함께 확인해주세요.</p>",
        ),
        "privacy.html": (
            "개인정보처리방침",
            "<p>본 블로그는 구글 애널리틱스(GA4) 및 구글 애드센스를 통해 방문자 통계와 광고를 제공할 수 있습니다. 이 과정에서 쿠키(Cookie)가 사용될 수 있으며, 쿠키를 통해 수집되는 정보에는 개인을 직접 식별할 수 있는 정보는 포함되지 않습니다.</p>"
            "<h2>쿠키 및 광고</h2>"
            "<p>구글을 포함한 제3자 광고 공급업체는 쿠키를 사용하여 사용자의 이전 방문 기록을 기반으로 광고를 게재합니다. 이용자는 <a href=\"https://adssettings.google.com\" target=\"_blank\">구글 광고 설정</a>에서 맞춤 광고를 비활성화할 수 있습니다.</p>"
            "<h2>문의</h2><p>개인정보 관련 문의사항은 문의하기 페이지를 통해 연락 주시기 바랍니다.</p>",
        ),
        "contact.html": (
            "문의하기",
